    ...
    def fitting(self, body_markers, object_contact, markers_contact, verts_object, normal_object, gender, betas=None):
        ...
        for ss, optimizer in enumerate(self.optimizers):
            for ii in range(self.num_iter[ss]):
                ...
                loss.backward(retain_graph=True) # TODO keep 'retain_graph'as True
                optimizer.step()
                ...
                if self.only_rec != True:
                    if ss==2 and ii>200 and eval_grasp < best_eval_grasp:# and contact_num>=5:
                        ...
                        tmp_smplxparams['transl'] = copy.deepcopy(self.transl_rec)
                        tmp_smplxparams['global_orient'] = RotConverter.rotmat2aa(RotConverter.cont2rotmat(copy.deepcopy(self.glo_rot_rec)))
                        # smplxparams['betas'] = torch.tensor(betas, dtype=torch.float32, requires_grad=False).repeat(self.batch_size, 1).to(self.device)
                        tmp_smplxparams['betas'] = copy.deepcopy(self.betas)
                        tmp_smplxparams['body_pose'] = self.vposer.decode(copy.deepcopy(self.vpose_rec),
                                                           output_type='aa').view(self.batch_size, -1)
                        tmp_smplxparams['left_hand_pose'] = copy.deepcopy(self.hand_pose)[:,:self.hand_ncomps]
                        tmp_smplxparams['right_hand_pose'] = copy.deepcopy(self.hand_pose)[:,self.hand_ncomps:]
                        tmp_smplxparams['leye_pose'] = copy.deepcopy(self.eye_pose)[:,:3]
                        tmp_smplxparams['reye_pose'] = copy.deepcopy(self.eye_pose)[:,3:]
            ...
            if not early_stop or tmp_info is None:
                ...
                smplxparams['transl'] = copy.deepcopy(self.transl_rec)
                smplxparams['global_orient'] = RotConverter.rotmat2aa(RotConverter.cont2rotmat(copy.deepcopy(self.glo_rot_rec)))
                smplxparams['betas'] = copy.deepcopy(self.betas)
                smplxparams['body_pose'] = self.vposer.decode(copy.deepcopy(self.vpose_rec),
                                                   output_type='aa').view(self.batch_size, -1)
                smplxparams['left_hand_pose'] = copy.deepcopy(self.hand_pose)[:,:self.hand_ncomps]
                smplxparams['right_hand_pose'] = copy.deepcopy(self.hand_pose)[:,self.hand_ncomps:]
                smplxparams['leye_pose'] = copy.deepcopy(self.eye_pose)[:,:3]
                smplxparams['reye_pose'] = copy.deepcopy(self.eye_pose)[:,3:]
