...
from flex.pretrained_models.loader_saga import Trainer as SAGA_Loader
...
from saga.utils.utils import rotmat2aa
...
class Optimize(RHGrasp_Loader, VPoser_Loader):
    ...
    def __init__(self, cfg_vp, cfg_rh):
        SAGA_Loader.__init__(self, cfg_rh)  # this sets the variable(s): self.coarse_net, self.refine_net
    ...
    def perform_optim(self, z_init, transl_init, global_orient_init, w_init, a_init, obstacles_dict,
                      obj_bps, object_mesh, obj_transl, obj_global_orient, obj_name, obj_info, model_name='flex'):
        """
    ...
    def get_inits(self, test_pose, obj_bps, bs):
        ...
        :return w_inits       (torch.Tensor) on device (bs, 16)  #saga latent 16D
        """
        ...
        w_inits = torch.zeros(bs, 16).to(self.device)
        return t_inits, g_inits, z_inits, w_inits


    def optimize(self, obj_name,object_info, test_pose, obstacles_dict, bs=1, model_name='flex'):
        ...
        curr_res = self.perform_optim(z_inits, t_inits, g_inits, w_inits, a_inits,
                                      ...
                                      obj_transl, obj_global_orient, obj_name, object_info, model_name)
        ...
        return results
    def get_objinfo_saga(pose,obj_name):
        transf_transl, global_orient = pose[0], pose[1]
        transl = torch.zeros_like(transf_transl) # for object model which is centered at object
        obj_meshes_dir = '/data/obj/contact_meshes'
        cuda_id = 0
        tmp_device = torch.device("cuda:%d" % cuda_id if torch.cuda.is_available() else "cpu")
        # get the object info
        obj_mesh_base = o3d.io.read_triangle_mesh(os.path.join(obj_meshes_dir, obj_name + '.ply'))
        obj_mesh_base.compute_vertex_normals()
        v_temp = torch.FloatTensor(np.asarray(obj_mesh_base.vertices)).to(tmp_device).view(1, -1, 3)
        normal_temp = torch.FloatTensor(np.asarray(obj_mesh_base.vertex_normals)).to(tmp_device).view(1, -1, 3)
        obj_model = ObjectModel(v_temp, normal_temp, 1)
        global_orient_rotmat = batch_rodrigues(global_orient.view(-1, 3)).to(tmp_device)  

        object_output = obj_model(global_orient_rotmat, transl.to(tmp_device), v_temp.to(tmp_device), normal_temp.to(tmp_device), rotmat=True)
        ...
        verts_object = torch.from_numpy(verts_object).to(tmp_device)
        feat_object = torch.from_numpy(feat_object).to(tmp_device)
        transf_transl = transf_transl.to(tmp_device)

        object_info = {'verts_object':verts_object, 'normal_object': normal_object, 'global_orient':global_orient, 
                    'global_orient_rotmat':global_orient_rotmat, 'feat_object':feat_object, 'transf_transl':transf_transl,'obj_name':obj_name}
        return object_info
...
def main():
    ...
    # === Load obj info for SAGA model input
    obj_info = tto.get_objinfo_saga(pose,obj_name)
    #=================================================================
    
    
    # === Run search given an input object, it's pose and obstacles info.
    res = tto.optimize(obj_name=obj_name,object_info=obj_info, test_pose=pose, obstacles_dict=obstacles_dict, bs=cfg_vp.bs, model_name=cfg_vp.model_name)
    final_results = []
